...
namespace PlayerController {
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        [SerializeField] private int detectorCount = 12;
        [SerializeField] private float detectionRayLength = 0.1f;
        [SerializeField] [Range(-0.02f, 0.3f)] private float rayBuffer = 0.1f; // Prevents side detectors hitting the ground
        ...
        private void CalculateWalk() {
            if (Input.X != 0) {
                ...
                var apexAppliedBonus = Mathf.Sign(Input.X) * apexBonus * _apexPoint;
                _currentHorizontalSpeed += apexAppliedBonus * Time.deltaTime;
            }
            else {
